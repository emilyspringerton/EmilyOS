...
EmilyOS 2 GUI v0.1 — Game-UI Filesystem Core premise
...
Visual system 1.1 Background + whitespace rules
...
Colors allowed: blue, magenta, teal, light teal, dark teal, red, yellow, green (You also said “all HTML colors like olive excluding white”—so the rule is: allow a standard HTML color set, but ban white & near-white; enforce luminance cap.)
...
Layout model: tmux × i3 hybrid 2.1 Screen is a tiling space
...
Interaction contract 3.1 No single-click affordances
Single click never triggers actions. Single click only does:
...
Filesystem mapping to tiles 4.1 Tile representations
...
Safety + policy hooks (EmilyOS posture compatibility) 5.1 Postures affect available actions, not the visuals
Under SIEGE/MERCY:
...
What’s intentionally missing (v0 scope control)
